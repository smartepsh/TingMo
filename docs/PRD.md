...
- **知识库纠正**（1.0 后）：用户过往转录、纠正结果、手动笔记、导入文本作为向量索引的语义检索源；LLM 纠正前按当前转录检索 top-K 相关片段注入 prompt；自动随使用积累，无需手动维护；所有数据本地存储
...
- 不支持 iOS / iPadOS
- 不实现语音命令控制（如"删除上一句"）
- 1.0 后知识库不上云，不做跨用户聚合——纯本地个性化
- v1 不做完整 Preset CRUD / 多 Preset 切换 / 按应用自动切换
- v1 不做自定义词典；知识库也放到 1.0 后
...
- **知识库（Knowledge Base）** — 1.0 后的本地语义检索源，内容包括历史转录、用户纠正、手动笔记、导入文本。LLM 纠正前通过 embedding 检索 top-K 注入 prompt。纯本地，默认关闭。
