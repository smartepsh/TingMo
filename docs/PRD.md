...
**个性化**
- 极简 Config Preset（v1）：仅承载 LLM 配置 + 知识库开关；完整 Preset（引擎/语言/设备模式/词典/多 Preset 切换）列为 1.0 后目标
...
- **极简 Config Preset（v1）** — 仅承载 LLM 配置 + 知识库开关，非完整 Preset。完整版（含引擎/语言/设备/词典切换）列为 1.0 后目标。
